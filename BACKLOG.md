...
- **HIGH: Перенос оркестрации в Java**
  Перенести в Java как можно больше последовательной оркестрации workflow.
  Первым шагом встроить уже существующий агентный фильтр в цепочку коллектора:

  ```text
  Java collector
  -> получает полный CollectionReport.scope
  -> запускает Python metrics_proxy.py через ProcessBuilder
  -> передаёт scope через stdin
  -> proxy читает analyzer/agent_filter.md и вызывает Codex CLI
  -> Java получает JSON с nonrelevant_job_ids
  -> проверяет, что ID принадлежат scope
  -> выставляет им NONRELEVANT
  -> возвращает наверх оставшийся упорядоченный scope
  ```

  Все собранные вакансии остаются в базе. Java использует существующий
  `metrics_proxy.py`, а не реализует заново вызов Codex и подсчёт токенов.
  Модель и effort по умолчанию берутся из `codex_proxy/config.ini`; отдельный
  вызов может передать override. `analyzer/agent_filter.md` остаётся независимым
  и может запускаться напрямую через Desktop или comparison mode для
  тестирования и настройки. Codex CLI должен быть авторизован и доступен в
  `PATH`; при запуске Java из будущего GUI не должно быть подтверждений Codex
  Desktop для сетевой команды.
