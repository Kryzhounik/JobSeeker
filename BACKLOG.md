...
- **CRITICAL: Преждевременная остановка локации Playwright-коллектором**
  Обнаружена ситуация, в которой LinkedIn-коллектор преждевременно считает
  локацию исчерпанной и прекращает её обход. Возможная причина — некорректная
  работа коллектора с `lazy` layout, но гипотеза пока не подтверждена.
  Диагностические логи прогонов и страниц уже добавлены. Исправления пока нет:
  сначала нужно собрать больше данных при следующем воспроизведении проблемы,
  зафиксировать layout и точную причину остановки.
...
- **HIGH: Перенос оркестрации в Java**
  ...
  агентным фильтром по названиям вакансий. Следующие этапы переносить отдельно
  после проверки этого контура.
