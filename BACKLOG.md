...
- **HIGH: Перенос оркестрации в Java**
  Перенести в отдельный Java-оркестратор как можно больше последовательной
  оркестрации workflow, не расширяя ответственность коллектора. Первым шагом
  добавить уже существующий агентный фильтр по названиям вакансий:

  ```text
  Java orchestrator
  -> запускает collector и получает полный упорядоченный scope
  -> запускает агентный фильтр для всего scope
  -> получает список нерелевантных вакансий
  -> проверяет, что ID принадлежат scope
  -> выставляет им NONRELEVANT
  -> передаёт дальше оставшийся упорядоченный scope
  ```

  Все собранные вакансии остаются в базе. Агентный фильтр остаётся независимой
  операцией, доступной для отдельного тестирования и настройки. Конкретный
  транспорт и способ связи Java с агентными операциями определяются при
  реализации оркестратора.
